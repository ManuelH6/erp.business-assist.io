...
namespace Workhub\Timesheet\Http\Controllers;
...
use Workhub\Timesheet\Http\Requests\StoreTimesheetRequest;
use Workhub\Timesheet\Http\Requests\UpdateTimesheetRequest;
use Workhub\Timesheet\Events\CreateTimesheet;
use Workhub\Timesheet\Events\UpdateTimesheet;
use Workhub\Timesheet\Events\DestroyTimesheet;
use App\Models\User;
...
class TimesheetController extends Controller
{
        ...
        if (Auth::user()->can('manage-timesheet')) {
            ...
            if (module_is_active('Taskly')) {
                $timesheets->getCollection()->transform(function ($timesheet) {
                    try {
                        if ($timesheet->project_id && class_exists('\Workhub\Taskly\Models\Project')) {
                            $project = \Workhub\Taskly\Models\Project::find($timesheet->project_id);
                            $timesheet->project_name = $project ? $project->name : 'N/A';
                        }
                        if ($timesheet->task_id && class_exists('\Workhub\Taskly\Models\ProjectTask')) {
                            $task = \Workhub\Taskly\Models\ProjectTask::find($timesheet->task_id);
            ...
            $projects = [];
            if (module_is_active('Taskly') && class_exists('\Workhub\Taskly\Models\Project')) {
                try {
                    if (Auth::user()->can('manage-any-timesheet')) {
                        $projects = \Workhub\Taskly\Models\Project::where('created_by', creatorId())
                            ->select('id', 'name')
                            ->get();
                    } else {
                        $projects = \Workhub\Taskly\Models\Project::whereHas('users', fn($q) => $q->where('user_id', Auth::id()))
                            ->select('id', 'name')
                            ->get();
                    }
                } catch (\Exception $e) {
    ...
    public function getAttendanceHours($userId, $date)
    {
        ...
        $attendance = \Workhub\Hrm\Models\Attendance::where('employee_id', $userId)
            ->whereDate('date', $date)
            ->first();
