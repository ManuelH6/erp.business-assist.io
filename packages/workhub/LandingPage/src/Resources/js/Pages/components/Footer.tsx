...
export default function Footer({ settings }: FooterProps) {
    ...
    const companyName = settings?.company_name || 'Assist Hub';
    const description = sectionData.description || 'La solution complète de gestion d\'entreprise pour les entreprises modernes.';
    const contactEmail = settings?.contact_email || 'support@assisthub.com';
    const phone = settings?.contact_phone || '+229 97 318 818';
    const newsletterTitle = sectionData.newsletter_title || 'Rejoignez notre communauté';
    ...
    const handleNewsletterSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (data.success) {
                toast.success(data.message || 'Merci de vous être abonné à notre newsletter !');
                setEmailInput('');
            } else {
                toast.error(data.message || 'Échec de l\'abonnement. Veuillez réessayer.');
            }
        } catch (error) {
        ...
        return (
            <div className={config.layout === 'split' ? 'space-y-6' : ''}>
                <Link href={route('landing.page')} className={config.companyName} style={config.layout !== 'split' ? { color: colors.primary } : {}}>
                    {logoUrl ? (
                        <img src={logoUrl} alt={companyName} className={config.layout === 'centered' ? 'h-12 w-auto object-contain mx-auto' : 'h-10 w-auto object-contain'} />
                    ) : (
                        companyName
                    )}
                </Link>
                ...
                <div className="space-y-3">
                    ...
                    <div className="flex items-center space-x-3">
                        <form onSubmit={handleNewsletterSubmit} className="flex items-center space-x-3">
                            ...
                            >
                                {isSubmitting ? 'Inscription...' : newsletterButtonText}
                            </button>
                        </form>
                    </div>
                </div>
