...
namespace Workhub\Pos\Http\Controllers;
...
use Workhub\Pos\Models\PosItem;
use Workhub\Pos\Http\Requests\StorePosRequest;
use Workhub\Pos\Events\CreatePosSale;
use Workhub\Pos\Models\PosPayment;
        ...
        $products = $query->get()
            ->map(function($product) {
                ...
                if ($product->tax_ids && is_array($product->tax_ids) && !empty($product->tax_ids)) {
                    $taxes = \Workhub\ProductService\Models\ProductServiceTax::whereIn('id', $product->tax_ids)
                        ->where('created_by', creatorId())
