    ...
    const downloadPDF = async () => {
        ...
        if (printContent) {
            const opt = {
                ...
                html2canvas: { scale: 2 },
                jsPDF: { unit: 'in', format: 'a4' as any, orientation: 'portrait' }
            };

            try {
                await html2pdf().set(opt as any).from(printContent as HTMLElement).save();
                        ...
                        <tbody>
                            {sale.items?.map((item, index) => (
                                <tr key={index} className="page-break-inside-avoid">
                                    ...
                                    <td className="text-right py-4 font-semibold">{formatCurrency(item.total_amount || 0)}</td>
                                </tr>
                            ))}
                        </tbody>
                    </table>
