...
namespace Workhub\Account\Helpers;
...
use Workhub\Account\Models\OpeningBalance;

class AccountUtility
{
    public static function defaultdata($company_id = null)
    {
        // ─── Switch : référentiel comptable configurable via .env ───────────
        // Ajoutez dans .env :  ACCOUNTING_FRAMEWORK=syscohada
        // Ajoutez dans config/app.php :
        //   'accounting_framework' => env('ACCOUNTING_FRAMEWORK', 'generic'),
        $framework = config('app.accounting_framework', 'generic');

        if ($framework === 'syscohada'
            && class_exists(\Workhub\Syscohada\Services\SyscohadaFramework::class)) {
            \Workhub\Syscohada\Services\SyscohadaFramework::defaultdata($company_id);
            return;
        }
        // ────────────────────────────────────────────────────────────────────

        // Comportement par défaut : plan comptable générique (GAAP/IFRS)
