...
class DashboardController extends Controller
{
    ...
    private function vendorDashboard()
    {
        ...
        if (class_exists('\\Workhub\\Account\\Models\\DebitNote')) {
            $recentDebitNotes = \Workhub\Account\Models\DebitNote::where('vendor_id', $user->id)
                ->latest()
                ->limit(5)
                ->get()
                ->map(function($note) {
    ...
    private function clientDashboard()
    {
        ...
        if (class_exists('\\Workhub\\Account\\Models\\CreditNote')) {
            $recentCreditNotes = \Workhub\Account\Models\CreditNote::where('customer_id', $user->id)
                ->latest()
                ->limit(5)
                ->get()
                ->map(function($note) {
