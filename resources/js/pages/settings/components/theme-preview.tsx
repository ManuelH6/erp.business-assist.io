...
export function ThemePreview({
  ...
  return (
    <div
      className={`border rounded-lg overflow-hidden text-xs transition-all duration-300 ${isDark ? 'bg-gray-900 text-white border-gray-700' : 'bg-background text-foreground'
        } ${isRTL ? 'rtl' : 'ltr'}`}
      style={{
    ...
    >
      ...
      <div className={`flex h-48 ${isRTL ? 'flex-row-reverse' : ''}`}>
        {/* Sidebar */}
        <div
          className={`order-1 rtl:order-2 ${getSidebarStyles()} ${sidebarVariant === 'floating' ? 'm-2 rounded-lg shadow-sm' : ''
